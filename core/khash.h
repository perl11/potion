...
#define KHASH_FLAG(name, h, i) *(uint32_t *)(((struct PNData *)h->table)->data + kh_flag_##name(i))
...
#define KHASH_INIT(name, khkey_t, khval_t, khkey_t2, kh_is_map, __hash_func, __hash_equal, __hash_func2, __hash_equal2) \
  ...
  static inline khkey_t *kh_key_##name(kh_##name##_t *h, khint_t i) { return (khkey_t *)(((struct PNData *)h->table)->data + kh_key_at_##name(i)); } \
  static inline khval_t *kh_val_##name(kh_##name##_t *h, khint_t i) { return (khval_t *)(((struct PNData *)h->table)->data + kh_val_at_##name(i)); } \
	static inline void kh_clear_##name(Potion *P, kh_##name##_t *h)				\
	{																	\
		if (h && h->table != PN_NIL) { \
	...
	static inline khint_t kh_get_##name(Potion *P, kh_##name##_t *h, khkey_t2 key)	\
	{																	\
		if (h->n_buckets) {												\
	...
	static inline void kh_resize_##name(Potion *P, kh_##name##_t *h, khint_t new_n_buckets) \
	{																	\
		...
		{																\
          ...
          int siz = sizeof(struct PNData) + kh_size_##name(new_n_buckets); \
          h->table = (PN)potion_gc_realloc(P, PN_TUSER, (struct PNObject *)h->table, siz); \
          ((struct PNData *)h->table)->siz = siz; \
        } \
			}															\
		}																\
		if (j) {														\
	...
	static inline khint_t kh_put_##name(Potion *P, kh_##name##_t *h, khkey_t key, int *ret) \
	{																	\
		khint_t x;														\
		if (h->n_occupied >= h->upper_bound) {							\
			if (h->n_buckets > (h->size<<1)) kh_resize_##name(P, h, h->n_buckets - 1); \
			else kh_resize_##name(P, h, h->n_buckets + 1);					\
		}																\
		{																\
	...
	static inline void kh_del_##name(Potion *P, kh_##name##_t *h, khint_t x)		\
	{																	\
...
#define kh_resize(name, h, s) kh_resize_##name(P, h, s)
