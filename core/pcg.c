/*
 * PCG Random Number Generation for C. Based on pcg_basic.c
 *
...
PN potion_srand(Potion *P, PN cl, PN self, PN seed) {
  pcg32_srandom_r(&pcg32_global, PN_INT(seed), 1); // TODO: a stream id
  return self;
}
...
/**\memberof Lobby
 ...
 \param bound PNInteger, default: 0xffffffff
 \return PNInteger in [0, bound) interval
...
/**\memberof PNNumber
  ...
  \code 0 rand #=> 0.xxxxxx \endcode
  \code 1 rand #=> 0.xxxxxx \endcode
  \code 2.0 rand #=> [0-1].xxxxxx \endcode
 \return PNDouble in [0.0, self) interval
 \sa potion_rand for long, potion_srand. */
PN potion_num_rand(Potion *P, PN cl, PN self) {
  double bound = PN_DBL(self);
  if (bound == 0.0 || bound == 1.0) {
    return potion_double(P, potion_rand_double());
  } else {
    if (bound == trunc(bound)) // 2.0 => [0-1] + dblrand
      return potion_double(P, pcg32_boundedrand((uint32_t)bound-1) + potion_rand_double());
    else {
      double x;
      do {
        x = pcg32_boundedrand((uint32_t)bound) + potion_rand_double();
      } while (x >= bound);
      return potion_double(P, x);
    }
  }
}
